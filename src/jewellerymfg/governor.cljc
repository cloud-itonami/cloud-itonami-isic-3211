(ns jewellerymfg.governor
  "Jewellery Workshop Plant Operations Governor -- the independent
  compliance layer that earns the JewelryAdvisor the right to commit.
  ...
  draft-schedule) casting/setting/polishing equipment, whether a
  proposal secretly tries to self-issue a hallmark or purity-assay
  ...
  coordination proposal and becomes direct casting/setting/polishing-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:jewellery-workshop-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       actuate` or `:setting-station/
                                       run`) is the 'direct casting/
                                       setting/polishing-line-
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       this (see `jewellerymfg.phase`:
                                       this op is never a member of any
                                       phase's `:auto` set either --
                                       two independent layers agree).
    5. Hallmark/purity-certification-
       authority blocked            -- ANY proposal (any op) whose own
                                       `:value`/`:patch` declares
                                       `:issue-hallmark-certification?
                                       true` is attempting to
                                       self-issue a hallmark / purity-
                                       assay certification -- an
                                       authority exclusively reserved
                                       to the accredited assay office /
                                       hallmarking authority, never
                                       this actor -- HARD, PERMANENT,
                                       unconditional.
   ...
   10. Invalid metal-type          -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:metal-type` outside the closed
                                       known set
                                       (`jewellerymfg.registry/metal-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated classification
                                       through.
   11. Invalid purity-permille     -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:purity-permille` that is not
                                       a physically/regulatorily
                                       plausible precious-metal
                                       fineness reading
                                       (`jewellerymfg.registry/purity-
                                       permille-valid?`), the batch
   ...
   12. Invalid weight-grams        -- for `:log-production-batch`, if
                                       ...
                                       physically plausible precious-
                                       metal-weight reading
                                       (`jewellerymfg.registry/weight-
   ...
   14. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [jewellerymfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  setting/polishing-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct casting/setting/polishing-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") may indicate direct casting/setting/polishing-equipment control, permanently blocked")}]))
...
(defn- actuate-equipment-blocked-violations
  ...
  to directly actuate casting/setting/polishing equipment -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate equipment directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-equipment? (:value proposal))))
    [{:rule :actuate-equipment-blocked
      :detail "direct casting/setting/polishing-equipment actuation is permanently blocked -- draft scheduling only"}]))

(defn- hallmark-authority-blocked-violations
  "HARD, PERMANENT, unconditional: ANY proposal (any op) whose own
  `:value`/`:patch` declares `:issue-hallmark-certification? true` is
  attempting to self-issue a hallmark / purity-assay certification --
  an authority exclusively reserved to the accredited assay office /
  hallmarking authority, never this actor. No phase and no human
  approval can ever override this."
  [proposal]
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-hallmark-certification? payload))
      [{:rule :hallmark-authority-blocked
        :detail "self-issuing a hallmark/purity-assay certification is permanently blocked -- exclusive authority of the accredited assay office"}])))
...
(defn- invalid-metal-type-violations
  "For `:log-production-batch`, if the patch declares a `:metal-type`
  outside the closed known set, reject rather than let a fabricated
  metal-type classification through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [metal-type (:metal-type (:value proposal))]
      (when (and (some? metal-type) (not (registry/metal-type-valid? metal-type)))
        [{:rule :invalid-metal-type
          :detail (str metal-type " is not a known metal-type value")}]))))

(defn- invalid-purity-violations
  "For `:log-production-batch`, if the patch declares a
  `:purity-permille` that is not a physically/regulatorily plausible
  precious-metal fineness reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [purity (:purity-permille (:value proposal))]
      (when (and (some? purity) (not (registry/purity-permille-valid? purity)))
        [{:rule :invalid-purity
          :detail (str purity " is outside the physically/regulatorily plausible purity-permille range")}]))))

(defn- invalid-weight-violations
  "For `:log-production-batch`, if the patch declares a
  `:weight-grams` that is not a physically plausible precious-metal-
  ...
  (when (= op :log-production-batch)
    (let [weight (:weight-grams (:value proposal))]
      (when (and (some? weight) (not (registry/weight-grams-valid? weight)))
        [{:rule :invalid-weight
          :detail (str weight "g is outside the physically plausible batch precious-metal-weight range")}]))))
...
(defn check
  "Censors a JewelryAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (hallmark-authority-blocked-violations proposal)
                           ...
                           (invalid-metal-type-violations request proposal)
                           (invalid-purity-violations request proposal)
