(ns jewellerymfg.advisor
  "JewelryAdvisor -- the *contained intelligence node* for the
  jewellery-workshop plant-operations coordination actor.

  It normalizes production-batch patches (metal-type/purity-permille/
  weight-grams/defect-rate), drafts a casting/setting/polishing-
  ...
  jewellery shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real casting/setting/polishing-
  equipment actuation, freight dispatch, or hallmark/purity-assay
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `jewellerymfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `jewellerymfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; setting/polishing-equipment-control
                                 ; effect
  ...
  `jewellerymfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [jewellerymfg.registry :as registry]
            [jewellerymfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's metal-type,
  purity-permille, weight-grams, or defect-rate, nor its verification
...
(defn- schedule-maintenance
  "Draft a casting/setting/polishing-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `jewellerymfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a materials-safety(solvent/acid)/theft-security/authenticity
  ...
  technicality). See `jewellerymfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `jewellerymfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound jewellery shipment coordination proposal against
  a production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `jewellerymfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "You are the advisor for a jewellery workshop plant-operations "
       "coordinator. Based only on the given facts, return exactly one "
       "proposal as an EDN map. Do not write any explanation or preamble, "
       "output EDN only.\n"
       ...
       "setting/polishing equipment (this actor only proposes, it never "
       "executes). Never propose self-issuing a hallmark or purity-assay "
       "certification. Never misreport shipment quantity."))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `jewellerymfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :jewellerymfg-advisor-proposal
   :op         (:op request)
