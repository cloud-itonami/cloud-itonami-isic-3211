(ns jewellerymfg.operation
  "JewelryOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (JewelryAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Jewellery
  Workshop Plant Operations Governor (:govern) and the rollout phase
  ...
  (:require [langgraph.graph :as g]
            ...
            [jewellerymfg.advisor :as advisor]
...
(defn build
  "Compiles a JewelryOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `jewellerymfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; JewelryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Jewellery Workshop Plant Operations Governor -- independent censor.
      (g/add-node :govern
