(ns jewellerymfg.store
  "SSoT for the jewellery-workshop plant-operations coordination
  ...
  (`cloud-itonami-isic-3250`'s own `medinstrmfg.store`), this build
  ...
  self-contained (no external jewellery-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             metal-type/purity-permille/weight-grams/
                             ...
                             file in the workshop's production ledger;
                             `:shipped-units` tracks the batch's own
                             cumulative-shipped ground truth.
    - `equipment`         -- a casting/setting/polishing unit's own
    ...
    - `maintenance`       -- a scheduled casting/setting/polishing-
                             equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`jewellerymfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound jewellery shipment
                             DRAFT (`jewellerymfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (jewellerymfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (jewellerymfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :metal-type :gold
                :lot-number "JWL-2026-001" :purity-permille 750
                :quantity-units 500.0 :weight-grams 1200.0
                :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :metal-type :platinum
                :lot-number "JWL-2026-002" :purity-permille 950
                :quantity-units 80.0 :weight-grams 900.0
                :defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :metal-type :silver
                :lot-number "JWL-2026-003" :purity-permille 925
                :quantity-units 600.0 :weight-grams 1800.0
                :defect-rate-percent 2.3
...
(defn- sample-equipment []
  {"casting-001" {:id "casting-001" :kind :casting-unit
   ...
   "setting-002" {:id "setting-002" :kind :setting-station
                   :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `jewellerymfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `jewellerymfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  casting unit (schedulable for maintenance), one UNVERIFIED/
  unregistered setting station (blocks any maintenance scheduling
