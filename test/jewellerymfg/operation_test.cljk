(ns jewellerymfg.operation-test
  "Smoke tests for the compiled JewelryOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `jewellerymfg.governor-contract-test`; the Store contract in
  `jewellerymfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "JewelryOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "casting-001" :severity :moderate :description "acid-pickling fume concern"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
