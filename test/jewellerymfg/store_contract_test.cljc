(ns jewellerymfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `jewellerymfg.store` ns docstring for why a second (Datomic-
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (false? (:verified? (store/equipment-unit s "setting-002"))))
    (is (false? (:registered? (store/equipment-unit s "setting-002"))))
    (is (= ["casting-001" "setting-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:metal-type :silver}})
    (is (= :silver (:metal-type (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "setting-002" :severity :high}})
