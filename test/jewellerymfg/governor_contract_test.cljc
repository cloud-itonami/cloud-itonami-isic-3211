(ns jewellerymfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control casting/setting/polishing
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue a hallmark/purity-assay
  certification') implemented faithfully. The single invariant under
  test:

    JewelryAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Jewellery Workshop Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:metal-type :gold}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :gold (:metal-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest hallmark-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-hallmark-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the hallmarking/purity-assay authority"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:hallmark-authority-blocked} (-> (store/ledger db) last :basis)))
      (is (not (true? (:issue-hallmark-certification? (store/batch db "batch-001"))))
          "fabricated self-hallmarking never lands in the SSoT"))))
...
(deftest invalid-metal-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:metal-type :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-metal-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:metal-type (store/batch db "batch-001"))) "fabricated metal-type never lands in the SSoT")))

(deftest invalid-purity-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:purity-permille 1500}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-purity} (-> (store/ledger db) last :basis)))
    (is (not= 1500 (:purity-permille (store/batch db "batch-001"))) "fabricated purity-permille never lands in the SSoT")))
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:metal-type :fabricated-metal}} coordinator)
